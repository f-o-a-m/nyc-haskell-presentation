...
getRichestNeighbors bn userAddress = do
  ...
  pure . take 10 . sortOn (Down . snd) $ pairs'
```
...
getRichestNeighborsK bn k userAddress = do
  traders <- getAllNs bn k userAddress
  pure . take 10 . sortOn (Down . down) $ pairs' 
  where
    getAllNs _ 0 _ = pure []
    getAllNs bn k userAddress = do
      traders <- getTraders userAddress
      ...
      let pairs' = filter ((> 0) . snd) pairs
          addrs = map fst pairs'
      rest <- concat <$> mapM (getAllNs bn (k-1)) addrs
      pure $ pairs' <> rest
```
